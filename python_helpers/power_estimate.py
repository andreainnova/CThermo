...
def generate_dH_df(fluid):
    ...
    for T_e in np.linspace(T_MIN, T_MAX, 20):
        for T_c in np.linspace(T_MIN, T_MAX, 20):
            try:
                dH_evap, dH_cond = get_dH_estimates(fluid, T_e, T_c)
            except:
                dH_evap, dH_cond = np.nan, np.nan
            df = pd.concat([df, pd.DataFrame({"T_e": [T_e], "T_c": [T_c], "dH_evap": [dH_evap], "dH_cond": [dH_cond]})])
    return df
...
if __name__ == "__main__":
    for gas in gas_list:
        ...
        model_evap, model_cond = ols("dH_evap ~ T_e + T_c", data=df).fit(), ols("dH_cond ~ T_e + T_c", data=df).fit()
